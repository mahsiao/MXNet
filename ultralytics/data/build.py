...
from ultralytics.data.mmdataset import YOLOMultiModalImageDataset
from ultralytics.data.loaders import (
    ...
    LoadTensor,
    SourceTypes,
    autocast_list, LoadMMImagesAndVideos,
)
...
def build_yolomm_dataset(cfg, img_path, batch, data, mode="train", rect=False, stride=32, multi_modal=False):
    """Build YOLOMM Dataset."""
    dataset = YOLOMultiModalDataset if multi_modal else YOLOMultiModalImageDataset
    return dataset(
        img_path=img_path,
        imgsz=cfg.imgsz,
        batch_size=batch,
...
def load_multimodal_inference_source(source=None, batch=1, vid_stride=1, buffer=False, imgsz=640):
    """
    ...
    if tensor:
        ...
        dataset = LoadMMImagesAndVideos(source, batch=batch, vid_stride=vid_stride, imgsz=imgsz)
