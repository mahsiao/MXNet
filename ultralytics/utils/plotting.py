...
class Annotator:
    ...
    def box_label(self, box, label: str = "", color: tuple = (128, 128, 128), txt_color: tuple = (255, 255, 255)):
        """Draw a bounding box on an image with a given label.在图像上绘制带有指定标签的边界框。
...
def plt_color_scatter(v, f, bins: int = 20, cmap: str = "viridis", alpha: float = 0.8, edgecolors: str = "none"):
    ...
    Args:
        v (数组类型): 用于绘制 x 轴的数值序列。
        f (数组类型): 用于绘制 y 轴的数值序列。
        bins (整数，可选): 直方图的分箱数目。
        cmap (字符串，可选): 散点图所使用的颜色映射表。
        alpha (浮点数，可选): 散点图的透明度系数。
        edgecolors (字符串，可选): 散点图标记的边框颜色。
