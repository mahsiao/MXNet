...
if __name__ == '__main__':
    model_path = 'ckpts/DV/DV-11n-mid-p3.pt'
    model = DualYOLO(model_path)  # 选择训练好的权重路径
    # print(model.model)
    result = model.val(data=r'datasets/DVOBB.yaml',
              split='val',
              imgsz=640,
              batch=16,
              use_simotm="RGBT",
              ...
              project='DVOBB',
              name='yolo11-mid-p3-obb',
              )

    # if model.task == 'detect':  # 仅目标检测任务适用
    if model.task == 'obb':  # 仅目标检测任务适用
