...
from ultralytics.data import load_inference_source,load_multimodal_inference_source
...
class BasePredictor:
    ...
    def setup_source(self, source, stride: int | None = None):
        ...
        # )
        self.dataset = load_multimodal_inference_source(
            source=source,
            batch=self.args.batch,
            vid_stride=self.args.vid_stride,
            buffer=self.args.stream_buffer,
    ...
    def write_results(self, i: int, p: Path, im: torch.Tensor, s: list[str]) -> str:
        ...
        # Add predictions to image / to RGB and IR image
        if self.args.save or self.args.show:
            ...
            self.plotted_img_ir = None
            if isinstance(result.orig_img, np.ndarray) and result.orig_img.ndim == 3 and result.orig_img.shape[2] == 4:
                ir_img = result.orig_img[:, :, 3:4]
                ir_img = np.repeat(ir_img, 3, axis=2)
                self.plotted_img_ir = result.plot(
                    line_width=self.args.line_width,
                    boxes=self.args.show_boxes,
                    conf=self.args.show_conf,
                    labels=self.args.show_labels,
    ...
    def save_predicted_images(self, save_path: Path, frame: int = 0):
        ...
        # Save images
            ...
            self.save_image_mutichannel(im, str(Path(save_path).with_suffix(".png")))
            if self.plotted_img_ir is not None:
                ir_save_path = save_path.with_name(f"{save_path.stem}_ir{save_path.suffix}")
                self.save_image_mutichannel(self.plotted_img_ir, str(Path(ir_save_path).with_suffix(".png")))
    def show(self, p: str = ""):
