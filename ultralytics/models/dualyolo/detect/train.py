...
from ultralytics.data import build_dataloader, build_yolo_dataset,build_yolomm_dataset
from ultralytics.engine.trainer import BaseTrainer
from ultralytics.models import yolo, dualyolo
from ultralytics.nn.tasks import DetectionModel
from ultralytics.utils import DEFAULT_CFG, LOGGER, RANK
from ultralytics.utils.patches import override_configs
from ultralytics.utils.plotting import plot_images, plot_labels, plot_results
from ultralytics.utils.torch_utils import de_parallel, torch_distributed_zero_first, unwrap_model
...
class DetectionTrainer(BaseTrainer):
    ...
    def build_dataset(self, img_path: str, mode: str = "train", batch: int | None = None):
        ...
        return build_yolomm_dataset(self.args, img_path, batch, self.data, mode=mode, rect=mode == "val", stride=gs)
    ...
    def get_validator(self):
        ...
        return dualyolo.detect.DetectionValidator(
...
class MultimodalDetectionTrainer(BaseTrainer):
    ...
    def plot_training_samples(self, batch, ni):
        """Plots training samples with their annotations, compatible with new plot_images."""

        # 1️⃣ 构造 labels 字典
        labels = {
            "cls": batch["cls"].squeeze(-1),  # shape: (batch, num_boxes)
            "bboxes": batch["bboxes"],  # shape: (batch, num_boxes, 4)
            "batch_idx": batch["batch_idx"],  # batch index for each box
            "img": batch["img"]  # image tensor
        }
        ...
        # 3️⃣ IR 图像（如果使用双模态）
        if getattr(self.args, "use_simotm", None) in ("RGBT", "RGBRGB6C"):
            # 1. 复制 labels
            labels_ir = labels.copy()

            # 2. 从 images tensor 中分离 IR 通道
            # 假设 batch["img"] 形状为 B x 4 x H x W，IR 通道在第 4 个通道
            ir_tensor = batch["img"][:, 3:4, :, :]  # 只取 IR
            # 如果需要显示成 3 通道，可以 repeat
            ir_tensor_rgb = ir_tensor.repeat(1, 3, 1, 1)  # B x 3 x H x W
            labels_ir["img"] = ir_tensor_rgb
            # 3. 调用新版 plot_images
            plot_images(
                labels=labels_ir,
                images=ir_tensor_rgb,  # 使用单独的 IR tensor
                paths=batch["im_file"],
                fname=self.save_dir / f"train_batch{ni}_ir.jpg",
                on_plot=self.on_plot,
                save=True,
