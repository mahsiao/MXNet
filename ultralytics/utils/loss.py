...
class ContrastFeatureLossMutualInfo(nn.Module):
    def __init__(self):
        super(ContrastFeatureLossMutualInfo, self).__init__()
    ...
    def mutual_information(self, x, y):
        ...
        x_centered = x - x_mean
        y_centered = y - y_mean
        cov_xy = torch.sum(x_centered * y_centered, dim=2) / (H * W)

        # 计算方差，加入更稳定的 epsilon
        var_x = torch.sum(x_centered ** 2, dim=2) / (H * W) + 1e-8
        var_y = torch.sum(y_centered ** 2, dim=2) / (H * W) + 1e-8

        # 互信息计算（避免数值不稳定）
        mi = cov_xy / (torch.sqrt(var_x) * torch.sqrt(var_y))
        return mi.mean()

    def forward(self, fea):
        total_loss = 0
        epsilon = 1e-6  # 防止数值不稳定

        for i in range(0, 9, 3):
            fea1, fea2, fea3 = fea[i], fea[i + 1], fea[i + 2]

            # 计算互信息并取反（使用 exp 代替 sigmoid 以防止梯度消失）
            mi1 = self.mutual_information(fea3, fea1)
            mi2 = self.mutual_information(fea3, fea2)
            mi_inv_loss = torch.exp(- (mi1 + mi2))  # 互信息越大，损失越小

            # print("mi_inv_loss:", mi_inv_loss.item())

            # 确保损失始终为正
            total_loss += mi_inv_loss
